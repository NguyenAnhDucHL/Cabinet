      ...
      <table className="w-full text-sm text-left text-gray-600">
        <thead className="text-xs text-gray-700 uppercase bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            {visibleColumns?.presider && (
              <th className="px-4 py-3 border-r border-gray-200">Chủ trì cuộc họp</th>
            )}
            {visibleColumns?.type && (
              <th className="px-4 py-3 border-r border-gray-200">Loại phiên họp</th>
            )}
            ...
            {visibleColumns?.actions && <th className="px-4 py-3 text-center">Hành động</th>}
          </tr>
        </thead>
        <tbody>
          {loading ? (
            <tr>
              <td colSpan={visibleCount || 7} className="px-4 py-8 text-center text-gray-500">
                Đang tải dữ liệu...
              </td>
            </tr>
            ...
            paginatedMeetings.map((m, index) => {
              ...
              return (
                ...
                >
                  ...
                  {visibleColumns?.location && (
                    <td className="px-4 py-3 text-gray-600">{m.location || m.roomName}</td>
                  )}
                  {visibleColumns?.presider && (
