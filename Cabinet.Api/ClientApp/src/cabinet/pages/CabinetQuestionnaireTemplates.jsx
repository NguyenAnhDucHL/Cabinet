...
import { useQuestionnaireTemplates } from '../../features/questionnaires/hooks/useQuestionnaireTemplates'

export function CabinetQuestionnaireTemplates() {
  const { templates, loading, fetchTemplates, create, update, remove } = useQuestionnaireTemplates()
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true)
    try {
      if (editingId) await update(editingId, templateName)
      else await create(templateName)
    ...
    <div className="flex flex-col h-full">
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col flex-1">
        ...
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            <h2 className="text-base font-bold text-gray-800">Mẫu phiếu lấy ý kiến</h2>
            <p className="text-xs text-gray-400 mt-0.5">Quản lý danh sách mẫu phiếu</p>
          </div>
          <button
