...
### [2026-09-08 08:56] Cấu hình chứng chỉ SSL cho Nginx
- **Mô tả**: Thiết lập Nginx sử dụng chứng chỉ SSL Wildcard cho `hopkhonggiay.vpdtcampha.vn`, chuyển hướng traffic HTTP (port 80) sang HTTPS (port 443). Bổ sung ngoại trừ `nginx/certs` khỏi GitHub Actions deploy `rsync` và `.gitignore` để bảo vệ private key khỏi việc vô tình xoá hoặc đẩy lên kho lưu trữ.
- **Tệp thay đổi**:
  - `nginx/conf.d/default.conf` (Sửa đổi cấu hình HTTPS)
  - `.github/workflows/deploy.yml` (Bổ sung rsync exclude)
  - `.gitignore` (Bổ sung quy tắc loại trừ certs)
- **Lệnh git commit**: `git commit -m "feat(infra): cấu hình ssl https cho domain hopkhonggiay"`
