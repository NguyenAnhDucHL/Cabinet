...
export function Search() {
  ...
  return (
    <div className="flex flex-col h-full gap-3 animate-in slide-in-from-bottom-4 duration-700 fill-mode-both">

      {/* ── Toolbar 2 dòng ─── */}
      <form onSubmit={handleSearch} className="glass-card rounded-xl px-4 py-2.5 shadow-sm flex flex-col gap-2 shrink-0">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="border-l-4 border-primary pl-3 mr-1 shrink-0">
            ...
            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider leading-none mt-0.5">Tra cứu nâng cao</p>
          </div>
          ...
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-1">
              <span className="size-3 inline-flex items-center justify-center text-primary text-[9px]">●</span>
              Trạng thái
            </span>
            <select
        ...
        <div className="flex items-center gap-2 border-t border-border/40 pt-2">
          ...
          <div className="ml-auto">
            <Button
              type="submit"
              className="h-8 px-4 rounded-lg bg-primary hover:bg-primary/90 text-white font-black uppercase tracking-widest text-[10px] shadow-md shadow-primary/20 transition-all active:scale-[0.98]"
              disabled={isLoading}
            >
              {isLoading ? <Loader2 className="size-3.5 animate-spin mr-1.5" /> : <SearchIcon className="size-3.5 mr-1.5" />}
              Tìm kiếm
            </Button>
          </div>
        </div>
        ...
        <CardContent className="flex-1 overflow-hidden flex flex-col p-0">
                ...
                {isLoading ? (
                  Array.from({ length: 5 }).map((_, i) => (
                    <TableRow key={i} className="h-[40px]">
                      <TableCell className="text-center w-14"><Skeleton className="h-4 w-6 mx-auto" /></TableCell>
                      <TableCell className="w-40"><Skeleton className="h-4 w-24" /></TableCell>
                      <TableCell className="w-36"><Skeleton className="h-4 w-24" /></TableCell>
                      <TableCell className="min-w-[300px]"><Skeleton className="h-4 w-full" /></TableCell>
                      <TableCell className="w-40"><Skeleton className="h-4 w-28" /></TableCell>
                      <TableCell className="w-36"><Skeleton className="h-6 w-24 rounded-full" /></TableCell>
                      <TableCell className="text-center w-24"><Skeleton className="h-7 w-7 rounded-full mx-auto" /></TableCell>
                    </TableRow>
                  ))
                ) : documents.length > 0 ? (
                  documents.map((doc, idx) => (
                    <TableRow key={doc.id} className="group transition-colors h-[40px] hover:bg-muted/30">
                      <TableCell className="py-1 text-muted-foreground font-medium text-[10px] w-14 text-center">
                        {(page - 1) * pageSize + idx + 1}
                      </TableCell>
                      <TableCell className="py-1 font-bold text-secondary w-40 truncate text-[11px]">{doc.soVanBan || '-'}</TableCell>
          ...
          <div className="px-4 py-2.5 border-t border-border flex items-center justify-between bg-card/50">
            <p className="text-xs text-muted-foreground font-medium">
              Trang <span className="text-foreground font-bold">{page}</span> / <span className="text-foreground">{totalPages || 1}</span>
